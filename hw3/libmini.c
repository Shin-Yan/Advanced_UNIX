...
int sigismember(const sigset_t *set, int signo){
	...
	unsigned int mask = sigmask(signo);
	return ((*set & mask) != 0);
}

sighandler_t signal(int signum, sighandler_t handler){
	if(handler == SIG_ERR || signal < 0 || signum > SIGNAL_MAX){
        set_errno(EINVAL);
        return SIG_ERR;
    }
...
int sigaction(int signum, struct sigaction *nact, struct sigaction *oact) {
	nact->sa_flags |= SA_RESTORER;
	nact->sa_restorer = __myrt/* your customized restore routine, e.g., __myrt */;
	long ret = sys_rt_sigaction(signum, nact, oact, SIGSET_SIZE);
	WRAPPER_RETval(int);
}
...
unsigned int sigmask(int signo){
	unsigned int mask = 1;
	return mask << (signo-1);
}

#if 1	/* we have an equivalent implementation in assembly */
